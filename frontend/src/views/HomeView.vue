...
const currentUserId = computed(() => (auth.user?.id ? String(auth.user.id) : ''));
...
function hasLiked(p: Post) {
  ...
  if (!id) return false;
  return Array.isArray(p.likedBy) && p.likedBy.some((entry) => String(entry) === id);
}
...
async function onLike(p: Post) {
  ...
  if (!isLoggedIn.value) {
    $q.notify({ type: 'warning', message: 'Inicia sesión para dar like' });
    void router.push('/login');
    return;
  }

  if (hasLiked(p)) {
    $q.notify({ type: 'info', message: 'Ya habías dado like a esta publicación' });
    return;
  }
  ...
  try {
    ...
    if (Array.isArray(res.likedBy)) {
      p.likedBy = res.likedBy;
    } else if (!hasLiked(p)) {
      p.likedBy = [...(p.likedBy ?? []), currentUserId.value];
    }
